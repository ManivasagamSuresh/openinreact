...
import { auth, provider } from '../AuthProvider/firebase';
...
const HandleGoogleLogin = async()=>{
  try {
     let googleuser = await signInWithPopup(auth,provider);
     
   
     if(googleuser.user.emailVerified){
      const user = await axios.get(`${config.api}/user/getUserByMail/${googleuser.user.email}`)
      ...
      if(user.data){
        ...
        const values = {
          ...
          "password":googleuser.user.email,
        }
        const result  = await axios.post(`${config.api}/user/signup`,values);
        const user = await axios.get(`${config.api}/user/getUser/${result.data.insertedId}`)
        setUser(user.data);
      }
      navigate("/dashBoard")
     }
...
const formik = useFormik({
  ...
  onSubmit:async(values)=>{
    try {
      setLoading(true)
      console.log(values)
      const result  = await axios.post(`${config.api}/user/signin`,values);
      console.log(result);
      formik.resetForm();
  ...
  {/* left panel */}
    ...
    <div className="right flexP bg-bgSoft flex flex-col  lg:ml-24 justify-center items-center lg:gap-5 px-10 md:py-5">
      ...
      <div className="rightContainer flex flex-col gap-2 lg:gap-7 w-full sm:w-4/5 md:w-3/5  lg:w-fit ">
        ...
        <div className={`text-2xl text-black font-bold monteserat-c`} >Sign In</div>
        <div className={`mb-4 md:mb-6 text-black lato-c`}>Sign in to your account</div>
        </div>
        ...
        Don’t have an account? <span className="text-link cursor-pointer"><Link to="/signup">Register here</Link>  </span>
        </div>
      </div>
      </div>
</div>
